...
export default function AdminLayout({
  ...
  const isRegularAdmin =
    userProfile?.role === UserRole.ADMIN && userProfile?.isActive;
  const isSuperAdmin =
  ...
  useEffect(() => {
    ...
    // If user is super admin, redirect to superadmin area
    if (user !== null && isLoggedIn && isAdmin && isSuperAdmin) {
      router.push("/superadmin");
      return;
    }

    // If user is admin but not regular admin, redirect to login
    if (user !== null && isLoggedIn && isAdmin && !isRegularAdmin) {
      router.push("/login");
      return;
    }
  }, [isLoggedIn, isAdmin, isRegularAdmin, isSuperAdmin, user, router]);
  ...
  if (!isLoggedIn || !isAdmin || !isRegularAdmin) {
    return (
      <div className="flex min-h-screen items-center justify-center">
        <div className="text-center">
          <Loader2 className="mx-auto h-8 w-8 animate-spin" />
